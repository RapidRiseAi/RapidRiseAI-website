...
To keep `/api/lead` working in production, deploy the Cloudflare Pages Function in `functions/api/lead.ts` alongside the static assets.
...
2. Add these as **Environment Variables** for each environment you use (Preview/Production):
   - `NEXT_PUBLIC_APPS_SCRIPT_BASE_URL` = your deployed Google Apps Script web app URL (or set `APPS_SCRIPT_BASE_URL` for server-only use in Pages Functions).
...
- **`/api/lead` returns 404 in production**: this happens if only static assets are deployed and the Pages Function was not published. Ensure `functions/api/lead.ts` is deployed with the site.
- **500 from `/api/lead`**: missing or incorrect `NEXT_PUBLIC_APPS_SCRIPT_BASE_URL` (or `APPS_SCRIPT_BASE_URL` when using Pages Functions).
