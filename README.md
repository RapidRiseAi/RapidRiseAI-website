...
- **500 from `/api/lead`**: missing/invalid Apps Script URL variable. Use one canonical variable only (recommended: `APPS_SCRIPT_BASE_URL`). Worker fallback order is `APPS_SCRIPT_BASE_URL` → `SCRIPT_EXEC_URL` → `NEXT_PUBLIC_APPS_SCRIPT_BASE_URL`, so an incorrect higher-priority value can override a correct lower-priority one.
