...
export function MobileOperationsPreview() {
  return (
    <div className="mt-8 rounded-[26px] border border-cyan-400/45 bg-slate-950/70 p-5 shadow-[0_18px_55px_rgba(2,6,23,0.7),0_0_26px_rgba(14,116,255,0.22)] backdrop-blur-lg">
      <div className="flex items-center justify-between">
        <p className="text-xs font-semibold tracking-[0.2em] text-white/85">OPERATIONS PREVIEW</p>
        <span className="inline-flex items-center gap-1.5 rounded-full border border-emerald-400/30 bg-emerald-500/15 px-3 py-1 text-sm font-medium text-emerald-300">
      ...
      <div className="mt-5 grid grid-cols-3 divide-x divide-white/15 rounded-xl border border-white/10 bg-slate-900/55 px-2 py-3 text-center">
        {heroMetrics.map((metric) => (
          <div key={metric.label} className="px-2">
            <p className={`text-4xl font-semibold tracking-[-0.03em] text-white ${metric.accent ?? ''}`}>{metric.value}</p>
      ...
      <div className="relative mt-5 h-36 overflow-hidden rounded-xl border border-white/10 bg-slate-950/85">
        <svg viewBox="0 0 340 144" className="absolute inset-0 h-full w-full" aria-hidden>
          <path d="M0 96 C24 96, 45 112, 64 108 C85 102, 108 96, 132 62 C148 44, 168 74, 186 78 C214 88, 238 92, 264 84 C286 75, 304 56, 340 62" stroke="#0ea5e9" strokeWidth="2.7" fill="none" className="drop-shadow-[0_0_6px_rgba(34,211,238,0.75)]" />
        ...
        {processSteps.map((step, index) => {
          ...
          return (
            <div key={step.label} className="contents">
              <div>
                <span className="mx-auto inline-flex h-9 w-9 items-center justify-center rounded-full border border-white/15 bg-slate-900/80">
                  <Icon className={`h-[18px] w-[18px] ${step.accent}`} />
                </span>
