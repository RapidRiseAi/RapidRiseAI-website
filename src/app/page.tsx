...
export default function HomePage() {
  return (
    <>
      ...
      <ManualVsSystems />
      <Section label="Process" title="How we work"><Steps items={[{title:'Diagnose the leaks',body:'We map the workflow and find where leads, time, and accuracy are being lost.'},{title:'Build the system',body:'We automate the repetitive work, connect tools, and build any internal pages or dashboards required.'},{title:'Improve and scale',body:'We refine the system based on what the data shows and what the team actually needs.'}]} /></Section>
      <Section label="Portfolio" title="Systems we build" intro="You do not need everything. You need the right system.">
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">{['Enquiry to quote pipeline','Follow-up reminders and escalation','Workspace automation for admin tasks','Internal dashboards and reporting','Onboarding and folder automation','Sample build: Hospitality enquiry assistant'].map((x, idx)=><Card key={x}><Image src={`/images/work/work-0${idx + 1}.svg`} alt="" width={400} height={240} className="mb-4 h-36 w-full rounded-xl border border-stroke object-cover" /><div className="flex items-center justify-between gap-2"><h3 className="font-[var(--font-jakarta)] font-semibold">{x}</h3>{x.includes('Sample')?<Pill className="border-blue/50 text-blue">Sample build</Pill>:null}</div><p className="mt-2 text-sm text-text1">Outcome focused delivery for a cleaner operation.</p><a href="/work" className="mt-3 inline-flex items-center gap-2 text-sm text-blue">View work <ArrowRight className="h-4 w-4" /></a></Card>)}</div>
      </Section>
      <Section label="Pricing" title="Simple packages. Clean delivery." intro="Start small, build the core, then scale."><PricingCards cards={[{title:'Quick Wins (2 weeks)',includes:[]},{title:'Core System (4 to 6 weeks)',includes:[]},{title:'Scale and Support (monthly)',includes:[]}]} /></Section>
      <Section label="FAQ" title="Questions we get a lot"><FAQAccordion items={[{q:'Do you work with our current tools?',a:'Yes. We usually improve what you already use before recommending changes.'},{q:'How fast can you deliver value?',a:'Quick Wins are designed to show value within 2 weeks.'},{q:'Do you build websites and web apps too?',a:'Yes. We build internal tools and public sites when it supports the workflow.'},{q:'Do you only do chatbots?',a:'No. Assistants are one part of a bigger system.'},{q:'What does handover look like?',a:'You get documentation, training, and ownership of your workflow.'}]} /></Section>
      <Section>
        <Card className="border-blue/40 p-8">
          <div className="grid gap-6 lg:grid-cols-[1fr_auto] lg:items-center">
            <div><p className="text-sm text-blue">Ready to start</p><h2 className="mt-2 font-[var(--font-jakarta)] text-3xl font-semibold">Stop the leaks. Build the system.</h2><p className="mt-3 text-sm text-text1">Response within 24 hours. No spam. No pressure.</p></div>
            <div className="flex flex-wrap gap-3"><Button href="/quote" arrow>Request a Quote</Button><Button href="/work" variant="secondary">View Work</Button></div>
          </div>
        </Card>
      </Section>
    </>
